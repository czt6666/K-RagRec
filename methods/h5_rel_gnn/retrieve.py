...
class GraphRetrieval:
    def __init__(self, model_name='sbert', path='dataset/fb'):
        ...
        # H5: derive a global per-edge relation id from edge_attr. Many edges
        # share the same SBERT relation embedding (one vector per relation
        # type), so torch.unique recovers a discrete relation vocabulary.
        _, self.global_edge_type = torch.unique(self.G.edge_attr, dim=0, return_inverse=True)
        self.num_relations = int(self.global_edge_type.max().item()) + 1
        print(f"[H5] derived {self.num_relations} relation types from G.edge_attr")
    ...
    def get_first_order_subgraph(self, node):
        ...
        kept_edge_idx = [edge_to_index[(src, dst)] for src, dst in subgraph_edges if (src, dst) in edge_to_index]
        subgraph_edge_features = self.G.edge_attr[kept_edge_idx]
        subgraph_edge_type = self.global_edge_type[kept_edge_idx]

        subgraph = Data(x=subgraph_node_features, edge_index=subgraph_edge_index, edge_attr=subgraph_edge_features, edge_type=subgraph_edge_type, num_nodes=len(subgraph_nodes))
