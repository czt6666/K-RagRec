...
class GraphLLM(torch.nn.Module):
    ...
    def encode_graphs(self, samples):
        ...
        graph_embeds_list = []
        # H5: relation-aware GNNs (RGCN/CompGCN) take an extra edge_type kwarg.
        # PyG's Batch.from_data_list auto-batches custom fields, so we just
        # forward edge_type when available.
        import inspect
        accepts_edge_type = 'edge_type' in inspect.signature(self.graph_encoder.forward).parameters

        for graphs in graphs_list:
            graphs = Batch.from_data_list(graphs).to(self.model.device)
            graphs = graphs.to(self.model.device)
            kwargs = {}
            if accepts_edge_type and hasattr(graphs, 'edge_type'):
                kwargs['edge_type'] = graphs.edge_type
            n_embeds, _ = self.graph_encoder(graphs.x, graphs.edge_index.long(), graphs.edge_attr, **kwargs)
