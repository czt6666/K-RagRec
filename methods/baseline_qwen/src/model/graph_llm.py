...
# ChatML wrapper for Qwen2.5-Instruct: the graph soft-prompt embedding is
# spliced in right after the "user\n" turn marker, mirroring how BOS/EOS_USER
# bracket it for Llama's [INST]...[/INST] format.
...
class GraphLLM(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(args.llm_model_path, use_fast=self.is_qwen, revision=kwargs["revision"])
        if self.is_qwen:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            args.llm_model_path,
            torch_dtype=torch.bfloat16 if self.is_qwen else torch.float16,
