...
class GraphLLM(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # H1: replace MLP-projector + mean-pool with a Graph-Q-Former that
        # cross-attends from learnable query tokens over the N retrieved
        # subgraph embeddings, emitting a fixed number of soft-prompt tokens.
        self.qformer = GraphQFormer(
            gnn_dim=args.gnn_hidden_dim,
            llm_dim=args.llm_hidden_dim,
    ...
    def forward(self, samples):
        ...
        # encode graphs -> Q-Former -> q soft-prompt tokens per sample
        graph_embeds_list = self.encode_graphs(samples)
        qformer_tokens_list = [self.qformer(g_embeds) for g_embeds in graph_embeds_list]
        ...
        for i in range(batch_size):
            ...
            # qformer_tokens_list[i] has shape (num_query_tokens, llm_dim)
            sample_graph_embeds = qformer_tokens_list[i]
