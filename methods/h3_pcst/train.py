    ...
    with open(train_data_path, 'r') as f:
        ...
        def train(inputs, questions=None, gold=None, targets=None, sequence_ids=None):
            ...
            for input, question, target, sequence_id in zip(inputs, questions, targets, sequence_ids):
                ...
                label.append(target)
                # H3: single PCST solve over the full KG using all history
                # items as anchors, replacing the baseline's per-item top-k.
                graph.append(retrieval_model.pcst_retrieval_topk(
                    input, sequence_id,
                    anchor_prize=args.pcst_anchor_prize,
                    topk_query_prize=args.pcst_topk_query_prize,
                    edge_cost=args.pcst_edge_cost,
                    max_subgraph_nodes=args.pcst_max_nodes,
                ))
        ...
        for epoch in range(args.num_epochs):
            ...
            epoch_log_path = f"{args.output_dir}/{args.dataset}/h3_pcst_train_log.json"
            ...
            summary_path = f"{args.output_dir}/{args.dataset}/h3_pcst_train_summary.json"
