...
def parse_args_llama():
    ...
    # H3 PCST retrieval
    parser.add_argument("--pcst_anchor_prize", type=float, default=10.0,
                        help="Prize on each history-item anchor node.")
    parser.add_argument("--pcst_topk_query_prize", type=int, default=20,
                        help="Top-K nodes by similarity also get cumulative prize.")
    parser.add_argument("--pcst_edge_cost", type=float, default=0.5,
                        help="Uniform per-edge cost; lower -> bigger subgraph.")
    parser.add_argument("--pcst_max_nodes", type=int, default=200,
                        help="Hard cap on subgraph node count.")
