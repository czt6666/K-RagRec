...
class GraphRetrieval:
    def __init__(self, model_name='sbert', path='dataset/fb'):
        ...
        self.G2 = self.load_third_layer_graph()  # H3: layer 3 (was unused in baseline)
        self.Graph = self.load_graph_data()
        self.sorted_item_ids=self.sort_item_list()
        self.movie_id_to_name = self.load_name()
        # H3: hop-field fusion of 3 KG layers (0/2/3) used as PCST node features
        from src.pcst_retrieval import fused_node_features, precompute_movie_anchors
        self.fused_x = fused_node_features(self.G.x, self.G1.x, self.G2.x)
        # H3: anchor cache (ML1M movie_id -> KG node id) precomputed once
        self.movie_id_to_anchor = precompute_movie_anchors(self)
        print(f"[H3] precomputed {len(self.movie_id_to_anchor)} movie anchors")
    ...
    def load_third_layer_graph(self):
        G_3 = torch.load(f'{self.path}/graphs/layer3_embeddings_W.pt', weights_only=False)
        return G_3

    def pcst_retrieval_topk(self, input_text, sequence_ids, anchor_prize=10.0,
                             topk_query_prize=20, edge_cost=0.5,
                             max_subgraph_nodes=200):
        """H3: replaces baseline retrieval_topk with one big connected
        subgraph from a single PCST solve. Returns a list with one Data so
        the rest of the pipeline (Batch.from_data_list) works unchanged.
        """
        from src.pcst_retrieval import pcst_retrieve
        sg = pcst_retrieve(
            self, input_text, sequence_ids,
            fused_x=self.fused_x,
            anchor_prize=anchor_prize,
            topk_query_prize=topk_query_prize,
            edge_cost=edge_cost,
