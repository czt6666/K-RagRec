...
def main(args):
    ...
    with open(train_data_path, 'r') as f:
        ...
        def train(inputs, questions=None, gold=None, targets=None, sequence_ids=None):
            ...
            graph=[]
            label=[]
            query_emb=[]   # H2: SBERT embedding of the recommendation prompt, fed to RetrievalGate
            sample = {'id':id,'graph':graph, 'question':query, 'label':label, 'query_emb':query_emb}
            d = 0
            for input, question, target, sequence_id in zip(inputs, questions, targets, sequence_ids):
                ...
                # SBERT-encode the watching history once per sample as the gate query
                q_emb = retrieval_model.encode_query(input).squeeze(0).detach()
        ...
        for epoch in range(args.num_epochs):
            ...
            epoch_log_path = f"{args.output_dir}/{args.dataset}/h2_gate_train_log.json"
            ...
            summary_path = f"{args.output_dir}/{args.dataset}/h2_gate_train_summary.json"
