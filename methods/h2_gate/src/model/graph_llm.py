...
class GraphLLM(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # H2: learnable retrieval gate. Scores each retrieved subgraph against
        # the user query embedding; the weighted mean replaces baseline's
        # plain mean over all retrieved subgraphs.
        self.gate = RetrievalGate(emb_dim=args.gnn_hidden_dim).to(self.model.device)
        self.gate_use_gumbel = bool(getattr(args, 'gate_use_gumbel', False))
        self.gate_tau = float(getattr(args, 'gate_tau', 1.0))
        self.gate_sparsity_lambda = float(getattr(args, 'gate_sparsity_lambda', 0.0))
        self._last_gate_sparsity_loss = torch.tensor(0.0)
    ...
    def encode_graphs(self, samples):
        ...
        query_embs = samples.get('query_emb', None)  # list[Tensor (1024,)] or None
        graph_embeds_list = []
        sparsity_terms = []
        ...
        for idx, graphs in enumerate(graphs_list):
            ...
            # H2: gate-weighted subgraph scoring.
            if query_embs is not None and query_embs[idx] is not None:
                q = query_embs[idx].to(self.model.device).to(g_embeds.dtype)
                weights, logits = self.gate(g_embeds, q)
                if self.training and self.gate_use_gumbel:
                    weights = self.gate.gumbel_sigmoid(logits, tau=self.gate_tau, hard=True)
                if self.gate_sparsity_lambda > 0.0:
                    sparsity_terms.append(weights.mean())
                g_embeds = g_embeds * weights.unsqueeze(-1)

            graph_embeds_list.append(g_embeds)

        if sparsity_terms:
            self._last_gate_sparsity_loss = self.gate_sparsity_lambda * torch.stack(sparsity_terms).mean()
        else:
            self._last_gate_sparsity_loss = torch.tensor(0.0, device=self.model.device)
    ...
    def forward(self, samples):
        ...
        # H2: optionally regularize the gate to prefer fewer retrievals
        return outputs.loss + self._last_gate_sparsity_loss.to(outputs.loss.device)
